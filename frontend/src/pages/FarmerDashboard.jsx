...
export default function FarmerDashboard() {
  ...
  const handleMarkCompleted = async (appId) => {
    try {
      await api.put(`/applications/${appId}/complete`);
      setRecentApplications(prev => prev.map(a => a._id === appId ? { ...a, status: 'completed' } : a));
      
      // Also update the local jobs state so the "My Jobs" column reflects the completion immediately
      const completedApp = recentApplications.find(a => a._id === appId);
      ...
      toast.success(isHi ? 'काम पूरा हुआ!' : 'Work marked as completed!');
    } catch (err) {
      console.error(err);
      toast.error(isHi ? 'स्थिति अपडेट करने में विफल' : 'Failed to mark as completed');
    }
  };
  ...
  return (
    <div className="page-container">
      <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.06 } } }}>
        ...
        {/* Bottom Grid: Let Recent Applications take full width since My Jobs is removed */}
            ...
            <motion.div variants={fadeUp} className="mb-6 lg:mb-0">
              ...
              <div className="flex flex-col gap-4">
                {recentApplications.map(app => (
                  <div key={app._id} className="card p-4 ring-1 ring-primary-100 dark:ring-primary-900">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-2.5 mb-3 flex flex-col gap-1">
                      <p className="text-xs font-semibold text-gray-900 dark:text-white">
                        {isHi ? 'के लिए आवेदन किया:' : 'Applied for:'} <span className="text-primary-600 dark:text-primary-400 cursor-pointer hover:underline" onClick={() => navigate(`/job/${app.job?._id}`)}>{app.job?.title}</span>
                      </p>
                    ...
                    {app.status === 'accepted' && (
                      <div className="flex gap-2">
                        ...
                        >
                           <FiCheck size={14} /> {isHi ? 'पूरा हुआ के रूप में चिह्नित करें' : 'Mark as Completed'}
                        </button>
                      </div>
                    )}
